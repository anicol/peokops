...
class GoogleReviewAnalysis(models.Model):
    ...
    categories = models.JSONField(
        default=list,
        help_text="High-level categories for this review (e.g., ['Food Quality', 'Service'])"
    )
...
class ReviewResponse(models.Model):
    """Manager response to a Google review (Phase 2: Review Response Management)

    Enables managers to reply to reviews directly from the platform.
    Supports AI-suggested responses and draft/publish workflow.
    """
    ...
    review = models.OneToOneField(
        ...
        help_text='The review this response is for'
    )
    ...
    created_by = models.ForeignKey(
        ...
        help_text='User who created this response'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    status = models.CharField(
        ...
        help_text='Draft, Published, or Failed'
    )
    published_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When the response was published to Google'
    )

    # AI assistance
    was_ai_suggested = models.BooleanField(
        default=False,
        help_text='Was this generated by AI?'
    )
    ai_suggestion_tone = models.CharField(
        max_length=20,
        choices=Tone.choices,
        null=True,
        blank=True,
        help_text='Tone used for AI suggestion'
    )

    # Google API tracking (for OAuth-synced reviews)
    google_reply_id = models.CharField(
        max_length=200,
        null=True,
        blank=True,
        help_text='Google reply ID from API (for OAuth reviews)'
    )
    error_message = models.TextField(
        null=True,
        blank=True,
        help_text='Error message if publish failed'
    )
    ...
    def __str__(self):
        return f'Response to {self.review} by {self.created_by}'

# ============================================================================
# Phase 3: Review Insights & Trending Issues
# ============================================================================

class ReviewTopicSnapshot(models.Model):
    """Snapshot of topic mentions for trend analysis (Phase 3)
    
    Stores aggregated topic data for time-series trending analysis.
    Created daily/weekly to track how topics trend over time.
    """
    ...
    account = models.ForeignKey(
        'accounts.Account',
        on_delete=models.CASCADE,
        related_name='topic_snapshots',
        help_text='Account this snapshot belongs to'
    )
    location = models.ForeignKey(
        ...
        blank=True,
        related_name='topic_snapshots',
        help_text='Specific location (null = account-wide)'
    )
    
    # Topic data
    topic = models.CharField(
        max_length=200,
        help_text='Topic/theme being tracked'
    )
    mention_count = models.IntegerField(
        default=0,
        help_text='Number of times mentioned in period'
    )
    positive_mentions = models.IntegerField(
        default=0,
        help_text='Mentions in positive reviews (4-5 stars)'
    )
    negative_mentions = models.IntegerField(
        default=0,
        help_text='Mentions in negative reviews (1-3 stars)'
    )
    avg_sentiment = models.FloatField(
        null=True,
        blank=True,
        help_text='Average sentiment score for this topic (-1 to 1)'
    )
    ...
    window_type = models.CharField(
        ...
        default='weekly',
        help_text='Aggregation window type'
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['account', 'topic', '-snapshot_date']),
            models.Index(fields=['location', 'topic', '-snapshot_date']),
            models.Index(fields=['snapshot_date', 'window_type']),
        ]
    ...
    def __str__(self):
        ...
        return f'{self.topic}{loc_str} on {self.snapshot_date}'


class TopicTrend(models.Model):
    """Calculated trend for a topic (Phase 3)
    
    Stores the calculated trend direction and velocity for topics.
    Updated periodically based on ReviewTopicSnapshot data.
    """
    ...
    account = models.ForeignKey(
        ...
        help_text='Account this trend belongs to'
    )
    location = models.ForeignKey(
        GoogleLocation,
        on_delete=models.CASCADE,
    ...
    category = models.CharField(
        ...
        null=True,
        blank=True,
        help_text='Category (food, service, cleanliness, etc.)'
    )
    overall_sentiment = models.CharField(
        max_length=20,
        choices=Sentiment.choices,
        default=Sentiment.NEUTRAL,
        help_text='Overall sentiment for this topic'
    )
    ...
    trend_velocity = models.FloatField(
        default=0.0,
        help_text='Rate of change (mentions per week)'
    )
    current_mentions = models.IntegerField(
        default=0,
        help_text='Mentions in current period'
    )
    previous_mentions = models.IntegerField(
        default=0,
        help_text='Mentions in previous period'
    )
    ...
    first_seen = models.DateTimeField(
        auto_now_add=True,
        help_text='When this topic was first detected'
    )
    last_updated = models.DateTimeField(
        auto_now=True,
        help_text='When trend was last calculated'
    )
    is_active = models.BooleanField(
        default=True,
        help_text='Whether this topic is still being mentioned'
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['account', '-current_mentions']),
            models.Index(fields=['location', '-current_mentions']),
            models.Index(fields=['trend_direction', '-current_mentions']),
            models.Index(fields=['overall_sentiment', '-current_mentions']),
